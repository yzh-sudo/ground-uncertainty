...
/*这段代码定义了一个名为`reflected_noise_removal`的函数，它是`PatchWorkpp`类的一个成员函数。这个函数的目的是从输入的点云中移除反射噪声。
函数接受两个参数：一个输入的点云`cloud_in`和一个非地面点云`cloud_nonground`。这两个参数都是引用，所以在函数中对它们的修改会影响到函数外部的对象。
在函数体中，首先遍历输入点云中的每个点。对于每个点，计算其在xy平面上的距离`r`，其z坐标，以及其与xy平面的夹角`ver_angle_in_deg`。
然后，检查这个点是否满足以下条件：夹角小于一个阈值[`RNR_ver_angle_thr_`]
总的来说，这个函数的目的是通过一些条件来识别并移除点云中的反射噪声。*/
template<typename PointT> inline
