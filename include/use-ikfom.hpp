...
/**double 是变量的标量类型。
98090 是球面的长度，表示球面上的点的数量。
10000 是球面的维度，表示球面上的点的坐标维度。
1 表示选择球面上的一个点作为旋转的原点
在状态变量中引入一个二维球面，用于表示可能存在的旋转变换*/
typedef MTK::vect<1, double> vect1;
typedef MTK::vect<2, double> vect2;

//宏定义的 state_ikfom //key_: state_ikfom
...
Eigen::Matrix<double, 24, 1> get_f(state_ikfom &s, const input_ikfom &in) {
    Eigen::Matrix<double, 24, 1> res = Eigen::Matrix<double, 24, 1>::Zero(); //过程矩阵
    vect3 omega;
    in.gyro.boxminus(omega, s.bg); //计算陀螺仪和偏置间的差值,将结果储存在omega中,即当前角速度
    vect3 a_inertial = s.rot * (in.acc - s.ba); //计算加速度和偏置间的差值,然后乘以s.rot得到惯性坐标系下的加速度,将结果储存在a_inertial中,即初始加速度
    for (int i = 0; i < 3; i++) {
        res(i) = s.vel[i];
        res(i + 3) = omega[i];
        res(i + 12) = a_inertial[i] + s.grav[i];
    }     //for循环将速度,角速度,加速度和重力加速度分别储存在res的对应位置中
    return res;
}

Eigen::Matrix<double, 24, 23> df_dx(state_ikfom &s, const input_ikfom &in) {
    Eigen::Matrix<double, 24, 23> cov = Eigen::Matrix<double, 24, 23>::Zero(); //24行23列的零矩阵
...
Eigen::Matrix<double, 24, 12> df_dw(state_ikfom &s, const input_ikfom &in) {
    Eigen::Matrix<double, 24, 12> cov = Eigen::Matrix<double, 24, 12>::Zero(); //24行12列的零矩阵
    cov.template block<3, 3>(12, 3) = -s.rot.toRotationMatrix();
    cov.template block<3, 3>(3, 0) = -Eigen::Matrix3d::Identity();
    cov.template block<3, 3>(15, 6) = Eigen::Matrix3d::Identity();
    cov.template block<3, 3>(18, 9) = Eigen::Matrix3d::Identity();
    return cov;
} //根据当前状态和输入计算状态转移函数对于噪声w的雅克比矩阵
