...
/*Eigen::aligned_allocator是Eigen定义的一个用于内存分配的自定义分配器类。这个分配器是为了确保在进行 SIMD（Single Instruction, Multiple Data）优化时数据的内存对齐*/
...
struct MeasureGroup {
    MeasureGroup() { this->lidar_.reset(new PointCloudType()); }; //.reset(new PointCloudType()) 是在调用 lidar_ 智能指针的 reset 方法，
    // 该方法销毁 lidar_ 当前指向的对象（如果有的话），并使其指向一个新分配的 PointCloudType 类型的对象。这里使用 new 关键字动态分配内存。
